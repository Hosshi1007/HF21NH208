	...
	throws ServletException,IOException{
		...
		try {
			...
			if(select_Beans.getHit_flag()) {//ユーザ名が存在するか確認
				...
				if(passwordStr.equals(row.get(2))) {
			...
			if(login){//ログイン成功
				...
				//セッションにバインド
				session.setAttribute("session_userID",row.get(0));//ログインID
				session.setAttribute("session_user_name",row.get(1));//ユーザー名
				//session.setAttribute("login_name",userStr);//ログイン名


				//homeで出すwaveを取得するメソッドを実行して、home.jspに送る準備をする。
				//Beanを呼び出してインスタンス化
				wave.wave_select_Beans get_wave = new wave.wave_select_Beans();

				//プロパティのセット
				//テーブル
				get_wave.setTable("t_user as user,t_wave as wave");
				//条件
				get_wave.setConditions("user.userID = wave.userID");
				//フィールド
				get_wave.setFields("user.userID,user_name,wave_contents,imgID1,imgID2,imgID3,imgID4,posttime,waveID");

				//Beanコール　準備OK
				get_wave.DBselect();

				//jspへ
				req.setAttribute("get_wave", get_wave);
